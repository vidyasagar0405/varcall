    ...
    def compose(self):
        ...
        with Container(id="Download_widget"):
            ...
            with Horizontal(id="Download_inputs"):
                ...
                yield Input(placeholder="outputfile name", id="Input_outputfile_name", suggester=FileSuggester(use_cache=False, case_sensitive=True))
        ...
        with Container(id="FastQC_widget"):
            ...
            yield Input(placeholder="Input file name", id="FastQC_Input", suggester=FileSuggester(use_cache=False, case_sensitive=True))
            with Horizontal(id="FastQC_Horizontal"):
                yield Label("FastQCs the files in the workingdir/data/reads directory if the input field is left empty", id="bwa_description")
        ...
        with Container(id="MultiQC_widget"):
            ...
            yield Input(placeholder="Input file name", id="MultiQC_Input", suggester=FileSuggester(use_cache=False, case_sensitive=True))
            with Horizontal(id="MultiQC_Horizontal"):
                yield Label("MultiQCs the files in the workingdir/results/fastqc directory if the input field is left empty \nNOTE: MultiQC can be used to visualise the output of various tools like samtools flagstat.\nFor more info check this", id="bwa_description")
        ...
        with Container(id="bwa_widget"):
            ...
            yield Input(placeholder="Input reads file name (2 files)", id="bwa_reads_Input", suggester=FileSuggester(use_cache=False, case_sensitive=True))
            yield Input(placeholder="Input reference genome file name", id="bwa_ref_Input", suggester=FileSuggester(use_cache=False, case_sensitive=True))
            with Horizontal(id="bwa_Horizontal"):
                yield Label("NOTE: The reference genome must be inedx before mapping\nIndexes all files in workingdir/data/reference directory if the input field is left empty\nMake sure to input two reads (1 sample)", id="bwa_description")
